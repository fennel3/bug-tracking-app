...
class IssueHydrator
{
    ...
    {
        $queryString = 'SELECT `issues`.`id`,`issues`.`title`, LEFT(`issues`.`description`,100) AS `summary` ,`issues`.`date_created`,`issues`.`reporter`,`issues`.`department`,`comments`.`issue_id`,COUNT(`comments`.`issue_id`) AS `comment_count`,`issues`.`completed`,`severities`.`name` AS `severity`
                FROM `issues`
